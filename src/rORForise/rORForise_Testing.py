import argparse
import evaluate_Testing


def parse_args():
    parser = argparse.ArgumentParser(description="rORForise: Process genome evaluation parameters.")
        ...
        for method in args.methods:
            ...
            for fragmentation_type in args.fragmentation_types:
                ...
                for group in args.subgroups:
                    total_preds, preds = evaluate_Testing.read_preds(args.directory, genome_name, method, fragmentation_type,
                                                             group)
                    print("Number of predictions made for reads", total_preds, sep="\t")
                    evaluate_Testing.evaluate(genome_name, args.gc_prob, preds, intersect_bed_filename, method)
